...
struct Constants {
    static let API_KEY = "471444777a35de2e49041eede5bdf8ee"
...
class APICaller {
    ...
    func getUpcomingMovies(completion: @escaping (Result<[Title], Error>) -> Void) {
        ...
        guard let url = URL(string: "\(Constants.baseURL)/3/movie/upcoming?api_key=\(Constants.API_KEY)&language=en=US&page1") else  { return }
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data , error == nil else {
                return
            }
        ...
        task.resume()
        //https://api.themoviedb.org/3/movie/upcoming?api_key=<<api_key>>&language=en=US&page1
    }
    
    
    func getPopular(completion: @escaping (Result<[Title], Error>) -> Void) {
        guard let url = URL(string: "\(Constants.baseURL)/3/movie/popular?api_key=\(Constants.API_KEY)&language=en=US&page1") else { return }
        let task = URLSession.shared.dataTask(with: URLRequest(url: url)) { data, _, error in
            guard let data = data, error == nil else {
                return
            }
    ...
    // https://api.themoviedb.org/3/movie/upcoming?api_key=<<api_key>>&language=en=US&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&with_watch_monetization_types=flatrate
